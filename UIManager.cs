...
public class UIManager : MonoBehaviour
{
    ...
    // State tracking
    private bool isLocalPlayerReady = false; // Renamed for clarity
    ...
    private void Awake()
    {
        ...
        Instance = this;
        // DontDestroyOnLoad is handled by GameSetup
        // --- END ADDED ---
        ...
        if (playerEntryTemplate != null)
            ...
            Debug.LogError("PlayerEntryTemplate is not assigned in the UIManager Inspector!");
    ...
    private void Start()
    {
        ...
        if (readyButton != null)
            ...
            Debug.LogError("ReadyButton is not assigned in the UIManager Inspector!");
        ...
        if (startGameButton != null)
        {
            ...
            Debug.LogError("StartGameButton is not assigned in the UIManager Inspector!");

        if (leaveButton != null)
            leaveButton.onClick.AddListener(OnLeaveButtonClicked);
         else 
            Debug.LogError("LeaveButton is not assigned in the UIManager Inspector!");
        ...
        if (playerNameInput != null)
        {
            ...
            Debug.LogError("PlayerNameInput is not assigned in the UIManager Inspector!");
    ...
    public void OnGameStateChanged(GameState newState)
    {
        ...
        switch (newState)
        {
            ...
            case GameState.StartScreen:
                 // Reset local ready state when returning to start screen
    ...
    private void OnConnectButtonClicked()
    {
        ...
        {
            // For now, let's assume we always try to create a lobby first
            // In a real game, you'd have UI to choose host/join/find lobbies
            Debug.Log("Attempting to create Steam Lobby...");
            SteamLobbyManager.Instance.CreateLobby();
            // GameManager.StartHost or JoinGame will be called by SteamLobbyManager callbacks
        }
        else
        {
            Debug.LogError("SteamLobbyManager instance not found. Cannot create/join lobby.");
            // Fallback to basic FishNet hosting if Steam fails?
            // Debug.Log("Falling back to basic FishNet hosting.");
    ...
    private void OnReadyButtonClicked()
    {
        ...
        if (localPlayer != null)
        {
            Debug.Log("Toggling ready state via NetworkPlayer");
        ...
        {
            Debug.LogError("Could not find local NetworkPlayer to toggle ready status.");
    ...
    // --- ADDED: Method to update button state/text based on boolean ---
    ...
    public void SetStartGameButtonActive(bool active)
    {
        if (startGameButton != null)
        {
             bool shouldBeVisible = active; // Host status is checked in GameManager
    ...
    // Add a new method to animate invalid button input
    ...
    // Helper to get the local NetworkPlayer
    private NetworkPlayer GetLocalNetworkPlayer()
    {
        Debug.Log($"Looking for local NetworkPlayer. Player count: {NetworkPlayer.Players.Count}");
        
        foreach (NetworkPlayer np in NetworkPlayer.Players)
        {
            if (np != null)
            {
                Debug.Log($"Found NetworkPlayer: {np.name}, IsOwner: {np.IsOwner}");
                if (np.IsOwner)
                {
                    Debug.Log($"Found local NetworkPlayer: {np.name}");
                    return np;
                }
            }
        }
        
        // If we get here, we couldn't find the local player
        Debug.LogWarning("No local NetworkPlayer found. NetworkPlayer.Players may be empty or no player has IsOwner=true");
        ...
        if (InstanceFinder.ClientManager != null && InstanceFinder.ClientManager.Connection.IsValid)
        {
            ...
            Debug.Log($"Attempting fallback with local ClientId: {localClientId}");
            
            foreach (NetworkPlayer np in NetworkPlayer.Players)
            {
                if (np != null && np.Owner.ClientId == localClientId)
                {
                    Debug.Log($"Found local NetworkPlayer via ClientId: {np.name}");
                    return np;
                }
            }
        }
